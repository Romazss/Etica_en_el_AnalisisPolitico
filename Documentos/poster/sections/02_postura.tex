...
\begin{center}
% Increased scale + circle size for better visual balance and to use page space
\begin{tikzpicture}[>=stealth, thick, scale=1.05, transform shape]
    % Nodos con tamaño fijo y ancho de texto ajustado para evitar desbordes
    \node[draw, circle, fill=uclight, minimum size=7.2cm, text width=6.2cm, align=center, drop shadow, font=\fontsize{18}{22}\selectfont] (pol) at (0, 3.5) {\textbf{POLARIZACIÓN}};
    
    \node[draw, circle, fill=uclight, minimum size=7.2cm, text width=6.2cm, align=center, drop shadow, font=\fontsize{18}{22}\selectfont] (des) at (7.4, -3.5) {\textbf{DESINFORMACIÓN}};
    
    \node[draw, circle, fill=ucred, text=white, minimum size=7.2cm, text width=6.2cm, align=center, drop shadow, font=\fontsize{18}{22}\selectfont] (mas) at (-7.4, -3.5) {\textbf{MÁS POLARIZACIÓN}};

    % Flechas con curvatura ajustada
    \draw[->, ucred, line width=3.8pt] (pol) to[bend left=25] (des);
    \draw[->, ucred, line width=3.8pt] (des) to[bend left=25] (mas);
    \draw[->, ucred, line width=3.8pt] (mas) to[bend left=25] (pol);
