...
\savebox{\sectionboxcontent}{%
        \begin{minipage}{0.95\linewidth}
            % Adjusted internal padding to save vertical space while keeping separation
            \vspace{20pt}
            \BODY
            \vspace{6pt}
        \end{minipage}%
...
\node[
    ...
    anchor=west,
    xshift=20pt
    ,yshift=6pt % lift the title slightly above the box to avoid overlap
] at (box.north west) { \vphantom{Ag} #2 };
\end{tikzpicture}
\end{center}
\vspace{0.4cm} % reduced post-section spacing to help fit to one page
}

\begin{document}
\shorthandoff{>}\shorthandoff{<}

\input{sections/01_header.tex}
...
\input{sections/02_postura.tex}
\input{sections/03_analisis.tex}
\input{sections/04_casos.tex}
% Forzar inicio de la siguiente sección en la columna central (columna 2)
\columnbreak

\input{sections/05_desinformacion.tex}
\input{sections/06_actores.tex}

\input{sections/07_riesgos.tex}
\input{sections/08_propuestas.tex}

\end{multicols}

\input{sections/09_reflexion.tex}
